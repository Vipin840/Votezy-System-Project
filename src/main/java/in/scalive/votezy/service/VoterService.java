...
public class VoterService {
    ...
    // Fixed registerVoter to handle hasVoted null
    public Voter registerVoter(Voter vote) {
        if (voterRepository.existsByEmail(vote.getEmail())) {
            throw new DuplicateResourceException("Voter with email id: " + vote.getEmail() + " already exists");
        }

        // Ensure hasVoted is not null
        if (vote.getHasVoted() == null) {
            vote.setHasVoted(false); // default false
        }
